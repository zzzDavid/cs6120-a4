...
def lvn(block, ins=None, debug=False):
    ...
    if ins != None:
        ...
        for in_tuple in ins:
            in_instr = dict()
            if in_tuple[1] == 'const':
                in_instr['dest'] = in_tuple[0]
                in_instr['op'] = in_tuple[1]
                in_instr['value'] = in_tuple[2]
    ...
    for idx, instr in enumerate(block):
        ...
        elif instr['op'] == 'const': # const instr
    ...
    # filter out the psuedo instrs
    new_block = [instr for instr in new_block if 'remove' not in instr]
    return new_block
